...
def binpath(name):
    p = shutil.which(name)
    if p: return p
    # Useful fallback if a bundled ffmpeg is present in the Python environment.
    try:
        import imageio_ffmpeg
        if name == "ffmpeg": return imageio_ffmpeg.get_ffmpeg_exe()
    except Exception:
        pass
    return None
...
def run_cmd(cmd):
    ...
    if not exe: raise RuntimeError(f"{cmd[0]} executable မတွေ့ပါ။ Streamlit app ကို rebuild/redeploy လုပ်ပါ။")
...
st.caption("Upload a video or use an authorized direct video URL.")
with st.sidebar:
...
if st.button("🚀 Generate Recap MP4",type="primary",use_container_width=True):
    if not video and not url: st.warning("Video upload သို့မဟုတ် direct video URL ထည့်ပါ။"); st.stop()
    try:
