...
def ffmpeg_exe():
    ...
    try:
        ...
        raise RuntimeError("FFmpeg မတွေ့ပါ။ imageio-ffmpeg ကို install လုပ်ပါ။") from exc
...
def translate_mm(text):
    ...
    mm = sum(1 for c in text if "\u1000" <= c <= "\u109f")
    if letters and mm / max(1, letters) > .35:
        return text
    from deep_translator import GoogleTranslator
    tr = GoogleTranslator(source="auto", target="my")
    pieces, cur = [], ""
    for word in text.split():
        if cur and len(cur) + len(word) + 1 > 3000:
            pieces.append(cur)
            cur = word
        else:
...
def make_srt(text, seconds, output):
    ...
    total, current, rows = sum(weights), 0.0, []
    def timestamp(v):
...
def download_video(url, output):
    """Use yt-dlp with a single best format first; fall back to FFmpeg merging when available."""
    errors = []
    try:
        import yt_dlp
        ffmpeg = ffmpeg_exe()
        format_choices = ["b", "best", "bv*+ba/b"]
        last = None
        for fmt in format_choices:
            try:
                ...
                with yt_dlp.YoutubeDL(opts) as ydl:
                    ...
                    candidates = [output, prepared, prepared.with_suffix(".mp4"), prepared.with_suffix(".webm"), prepared.with_suffix(".mkv")]
                    found = next((p for p in candidates if p.exists() and p.stat().st_size), None)
                    if found:
                        if found != output:
                            output.unlink(missing_ok=True)
                            found.replace(output)
                        last = None
                        break
                    last = RuntimeError("yt-dlp download ပြီးပေမယ့် output file မတွေ့ပါ။")
            except Exception as exc:
                last = exc
    ...
    try:
        ...
        if "text/html" in response.headers.get("content-type", "").lower():
            raise RuntimeError("Direct video URL မဟုတ်ပါ။ yt-dlp error: " + (errors[-1] if errors else "unknown"))
        size = 0
...
def youtube_search(query, token="", limit=20):
    ...
    if r.status_code != 200:
        raise RuntimeError(data.get("error", {}).get("message", f"YouTube API HTTP {r.status_code}"))
    items = []
    for x in data.get("items", []):
        vid = x.get("id", {}).get("videoId")
        if vid:
            s = x.get("snippet", {})
            items.append({"title": s.get("title", ""), "channel": s.get("channelTitle", ""), "url": f"https://www.youtube.com/watch?v={vid}"})
...
for i, item in enumerate(st.session_state.yt_results):
    ...
    if c2.button("သုံးမယ်", key=f"yt_{i}"):
        st.session_state.selected_url = item["url"]; st.rerun()
...
if st.button("🚀 Generate Myanmar Movie Recap", type="primary", use_container_width=True):
    if not upload and not url.strip():
        st.error("❌ Video Upload လုပ်ပါ သို့မဟုတ် Video Link ထည့်ပါ။"); st.stop()
    try:
